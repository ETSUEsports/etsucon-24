...
$("#resetBracketVAL").click(() => {
    if(!confirm("Are you sure you want to reset the bracket?")) {
        return;
    }
    nodecg.sendMessage('bracketVALReset');
});
...
globalTeams.on('change', (newVal) => {
    ...
    nodecg.readReplicant('bracketVAL', (data, err) => {
        if (err) {
            console.error(err);
        }
        for (let i = 1; i < 8; i++) {
...
$("#submitBracketVAL").click(() => {
    ...
    localTeams.push({
        "name": "DEFAULT_TEAM_1",
        "logo": "/bundles/etsucon-24/graphics/img/bracket-val/unknown.png",
        "color": "#bbbbbb"
    });
    ...
    if(winner.name != "NO_TEAM") {
        ...
        winner.logo = "/bundles/etsucon-24/graphics/img/bracket-val/unknown-black.png";
        winner.color = "#bbbbbb";
        winner.score = 0;
    }
    ...
    bracketVAL.value = transformedData;
});