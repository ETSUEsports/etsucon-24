...
globalTeams.on('change', (newVal) => {
    ...
    nodecg.readReplicant('bracketRL', (data, err) => {
        if (err) {
            console.error(err);
        }
        for (let i = 1; i < 8; i++) {
...
$("#submitBracketRL").click(() => {
    ...
    localTeams.push({
        "name": "DEFAULT_TEAM_1",
        "logo": "/bundles/etsucon-24/graphics/img/bracket-rl/unknown.png",
        "color": "#bbbbbb"
    });
    ...
    if(winner.name != "NO_TEAM") {
        ...
        winner.logo = "/bundles/etsucon-24/graphics/img/bracket-rl/unknown-black.png";
        winner.color = "#bbbbbb";
        winner.score = 0;
    }
    ...
    bracketRL.value = transformedData;
});